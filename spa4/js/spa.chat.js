...
spa.chat = (function(){
	...
	jqueryMap = {}, scrollChat, writeChat, writeAlert, clearChat, 
	...
	onClickToggle, configModule, initModule, removeSlider, handleSlider;
	...
	setSliderPosition = function (position_type, callback) {
		...
		jqueryMap.$slider.animate({height : height_px}, animate_time, 
			function(){
			jqueryMap.$toggle
				.prop('title', slider_title)
				.text(toggle_text);
	...
	writeChat = function (person_name, is_user) {
		var msg_class = is_user ? 'spa-chat-msg-log-me' : 'spa-chat-msg-log-msg';
		jqueryMap.$msg_log.append('<div class="'+msg_class+'">'+spa.util_b.encodeHtml(person_name)
	...
	onSetchatee = function (event, arg_map) {
		...
		if(!new_chatee){
			if(old_chatee){
				writeAlert(old_chatee.name + ' 离开了聊天室');
			} else {
				writeAlert('你的朋友已经走了！');
			}
			jqueryMap.$title.text('来啊来啊，来逗我啊！');
			return false;
		}
		jqueryMap.$list_box.find('.spa-chat-list-name')
				.removeClass('spa-x-select').end()
				.find('[data-id' + arg_map.new_chatee.id + ']')
				.addClass('spa-x-select');
		writeAlert('正在和 ' + arg_map.new_chatee.name + '会话');
		return true;
	};
	
	onListchange = function (event) {
		var list_html = String(),people_db = configMap.people_model.get_db(),
		...
		if(!list_html){
			list_html += '<div class="spa-chat-list-note">'
				  ...
				  + 'No one is online' + '</div>';
			clearChat();
		}
	};
	
	onUpdatechat = function (event, msg_map) {
		var is_user, sender_id = msg-map.sender_id, msg_text = msg_map.msg_text,
		chatee = configMap.chat_model.get_chatee() || {},
		sender = configMap.people_model.get_by_cid(sender_id);
		if(!sender){
			writeAlert(msg_text);return false;
		}
