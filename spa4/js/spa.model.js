...
spa.model = (function(){
	...
	chat = (function(){
		...
		_update_list, _leave_chat, get_chatee, 
		join_chat, send_msg, set_chatee, update_avatar, chatee = null;
		...
		_publish_updatechat = function (arg_list) {
			...
			if(!chatee){set_chatee(msg_map.sender_id);}
				...
				&& msg_map.sender_id !== chatee.id){set_chatee(msg_map.sender_id);}
		...
		set_chatee = function (person_id) {
			...
			} else {new_chatee = null;}
			$.gevent.publish('spa-chatee', {old_chatee : chatee, new_chatee : new_chatee});
			chatee = new_chatee;
			return true;
		};
		return {
